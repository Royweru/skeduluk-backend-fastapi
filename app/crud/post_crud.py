...
from app.utils.scheduling_utils import validate_scheduled_time, SchedulingError
...
class PostCRUD:
    ...
    async def get_posts_by_user(
        ...
        query = query.offset(skip).limit(
            limit).order_by(models.Post.created_at.desc())
    ...
    async def create_post(
        ...
        if post.scheduled_for:
            has_video = bool(post.video_urls and len(post.video_urls) > 0)
            # Convert to timezone-naive for validation
            scheduled_naive = make_timezone_naive(post.scheduled_for)
            # Validate the scheduled time meets minimum requirements
        ...
        if post.platform_specific_content:
            platform_specific_content_str = json.dumps(
                post.platform_specific_content)
    ...
    async def get_post_analytics(
        ...
        for post in posts:
            # Status counts
            status_distribution[post.status] = status_distribution.get(
                post.status, 0) + 1

            # Platform counts
            for platform in post.platforms:
                platform_distribution[platform] = platform_distribution.get(
                    platform, 0) + 1

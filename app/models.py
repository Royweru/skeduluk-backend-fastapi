...
class PostResult(Base):
    ...
    post_id = Column(Integer, ForeignKey(
        "posts.id", ondelete="CASCADE"), nullable=False, index=True)
...
class PostTemplate(Base):
    ...
    folder_id = Column(Integer, ForeignKey(
        "template_folders.id", ondelete="SET NULL"), nullable=True)
...
class TemplateAnalytics(Base):
    ...
    template_id = Column(Integer, ForeignKey(
        "post_templates.id", ondelete="CASCADE"), nullable=False, index=True)
    post_id = Column(Integer, ForeignKey(
        "posts.id", ondelete="SET NULL"), nullable=True)
    ...
    template = relationship(
        "PostTemplate", back_populates="template_analytics")
...
class UserAnalyticsSummary(Base):
    ...
    __table_args__ = (
        sa.UniqueConstraint('user_id', 'period', 'start_date',
